...
internal static class InitDepartments {
    ...
    private static Action<int, Func<int, string, DepartmentData>> add;
    internal static List<DepartmentData> departments {
        get {
            var l = new List<DepartmentData> {
    ...
    internal static void init(SchoolContext c, Action<int, Func<int, string, DepartmentData>> a) {
        db = c;
        departmentIDs = new Dictionary<string, int>();
        add = a;
    }
    internal static DepartmentData department(int idx, string year) =>
          department($"Department{idx}", (idx + 1) * 2000, $"{year}-09-01",
              $"LastName{idx * cntDepartmentMembers}");
    internal static DepartmentData department(string name, decimal budget, string date, string instructor)
