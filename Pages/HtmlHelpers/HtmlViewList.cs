...
public static class HtmlViewList {
    public static IHtmlContent ViewList<TModel> (
        this IHtmlHelper<IEnumerable<TModel>> h, IEnumerable<TModel> items, 
              Dictionary<string, Expression<Func<TModel, dynamic>>> columns, Func<TModel,dynamic> getId) {
        
        var s = htmlStrings(h, items, columns, getId);
        return new HtmlContentBuilder(s);
    }
    internal static List<object> htmlStrings<TModel>( IHtmlHelper<IEnumerable<TModel>> h, IEnumerable<TModel> items, 
        Dictionary<string, Expression<Func<TModel, dynamic>>> columns, Func<TModel, dynamic> getId) {
